...
export function createApp({ db, config = {} }) {
  ...
  app.get('/app/inbox', protect('administrator', 'operator', 'reviewer'), async (req, res, next) => { try {
    ...
    const rows = await db.query(`SELECT * FROM (
        SELECT i.id, i.title, i.canonical_url, i.updated_at, s.name AS source_name,
      ...
      ORDER BY inbox.updated_at DESC`, [req.user.workspaceId, query, sourceFilter, stateFilter]);
